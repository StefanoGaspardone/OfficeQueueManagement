...
export default function InfoDisplay({ ticketId }) {
    ...
    useEffect(() => {
        API.getCounters().then((counterIds) => {
            ...
            if (Array.isArray(counterIds)) {
                res = counterIds.map((counterId) => ({ "counterId": counterId, "lastTicketCalled": null }));
            } else {
                res = [];
            }
            setCounters(res);
    ...
    useEffect(() => {
        ...
        newSocket.on("newTicket", (data) => {
            if (waitingTicketsRef.current.includes(data.ticketId)) {
                console.log("Error: a ticket has been created but was already in the queue, ticket created is:", data.ticketId, "the queue is:", waitingTicketsRef.current);
            } else {
                ...
                <div className="counters-grid">
                    {counters.map((counter) => (
                        <div key={counter.counterId} className="counter-info-box">
                            ...
                            <p>{counter.lastTicketCalled ? `Serving: ${counter.lastTicketCalled}` : "Available"}</p>
                        </div>
                    ))}
                </div>
            </div>
